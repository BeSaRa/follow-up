    ...
    <div class="w-[32rem] max-w-full">
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4 px-6 py-5">
        <div class="grid grid-cols-2 gap-4">
          <ui-form-field>
            <label uiLabel>{{ 'priority_level.ar_name' | translate }}</label>
            <input uiInput formControlName="arName" />
          </ui-form-field>

          <ui-form-field>
            <label uiLabel>{{ 'priority_level.en_name' | translate }}</label>
        ...
        <div class="grid grid-cols-2 gap-4">
          <ui-form-field>
            <label uiLabel>{{ 'priority_level.lookup_key' | translate }}</label>
            <input uiInput type="number" formControlName="lookupKey" />
          </ui-form-field>

          <ui-form-field>
            <label uiLabel>{{ 'priority_level.category' | translate }}</label>
            <input uiInput type="number" formControlName="category" />
          </ui-form-field>
        </div>

        <ui-form-field>
          <label uiLabel>{{ 'priority_level.lookup_str_key' | translate }}</label>
...
export class PriorityLevelDialog implements OnInit {
  readonly dialogRef = inject(MatDialogRef<PriorityLevelDialog>)
  readonly data = inject<CrudDialogData<PriorityLevel>>(MAT_DIALOG_DATA)
  ...
  get titleKey() {
    switch (this.data.mode) {
      case 'CREATE':
        return 'priority_level.add'
      case 'UPDATE':
        return 'priority_level.edit'
      case 'VIEW':
        return 'priority_level.view'
    }
  }
  ...
  onSubmit() {
    ...
    const model = this.data.model ?? new PriorityLevel()
    this.dialogRef.close(model.clone<PriorityLevel>(this.form.value))
  }
}
